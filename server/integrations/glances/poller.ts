...
export async function poll(instance: PluginInstance): Promise<GlancesData | null> {
    ...
    try {
        ...
        let cpu = cached?.cpu ?? 0;
        let memory = cached?.memory ?? 0;
        let uptime = cached?.uptime ?? '--';
        ...
        // Cache this result for future fallback
        instanceCache.set(instance.id, result);

        return result;
    } catch (error) {
        const err = error as { message?: string };
        logger.debug(`[Poller:glances] Poll failed: ${err.message}`);
        return null;
    }
}
