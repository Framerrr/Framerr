...
export class OverseerrAdapter implements PluginAdapter {
    ...
    async execute(instance: PluginInstance, request: ProxyRequest): Promise<ProxyResult> {
        ...
        try {
            logger.debug(`[Adapter:overseerr] Request: method=${request.method} path=${request.path}`);
            const response = await axios(config);
            return { success: true, data: response.data };
        } catch (error) {
            const axiosError = error as { response?: { status: number; data?: unknown }; message: string };
            logger.error(`[Adapter:overseerr] Failed: error="${axiosError.message}" status=${axiosError.response?.status}`);
            return {
                success: false,
