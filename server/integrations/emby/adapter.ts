...
export class EmbyAdapter implements PluginAdapter {
    ...
    getAuthHeaders(instance: PluginInstance): Record<string, string> {
        ...
        // Emby uses X-Emby-Token header instead of MediaBrowser Authorization
        return {
            'X-Emby-Token': apiKey,
    ...
    async execute(instance: PluginInstance, request: ProxyRequest): Promise<ProxyResult> {
        ...
        try {
            logger.debug(`[Adapter:emby] Request: method=${request.method} path=${request.path}`);
            const response = await axios(config);
            return { success: true, data: response.data };
        } catch (error) {
            const axiosError = error as { response?: { status: number; data?: unknown }; message: string };
            logger.error(`[Adapter:emby] Failed: error="${axiosError.message}" status=${axiosError.response?.status}`);
            return {
                success: false,
