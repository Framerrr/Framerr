...
export class RadarrAdapter implements PluginAdapter {
    ...
    async execute(instance: PluginInstance, request: ProxyRequest): Promise<ProxyResult> {
        ...
        try {
            logger.debug(`[Adapter:radarr] Request: method=${request.method} path=${request.path}`);
            const response = await axios(config);
            return { success: true, data: response.data };
        } catch (error) {
            const axiosError = error as { response?: { status: number; data?: unknown }; message: string };
            logger.error(`[Adapter:radarr] Failed: error="${axiosError.message}" status=${axiosError.response?.status}`);
            return {
                success: false,
