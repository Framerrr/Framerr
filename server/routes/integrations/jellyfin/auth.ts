 ...
 * 
 * Endpoint:
 * - POST /authenticate - Authenticate with Jellyfin credentials (admin only)
 */
 ...
 */
router.post('/jellyfin/authenticate', requireAuth, requireAdmin, async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        logger.info(`[Jellyfin Auth] Authenticating: url="${baseUrl}" username="${username}"`);
        ...
        if (!User.Policy.IsAdministrator) {
            logger.warn(`[Jellyfin Auth] Non-admin login attempt: username="${username}"`);
            res.status(403).json({
                success: false,
                error: 'Admin account required. Please use a Jellyfin administrator account.',
            });
            return;
        }

        logger.info(`[Jellyfin Auth] Success: username="${User.Name}" userId="${User.Id}" isAdmin=true`);
        ...
        if (status === 401) {
            logger.warn(`[Jellyfin Auth] Invalid credentials: username="${username}"`);
