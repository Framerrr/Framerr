 ...
 * 
 * Fetches and indexes all media from a Jellyfin server into the local database.
...
import { JellyfinMediaItem, JellyfinView } from './types';
 ...
 */
export async function runJellyfinSync(
    ...
    try {
        ...
        for (let vi = 0; vi < allViews.length; vi++) {
            ...
            // Paginated fetch — avoids timeout on large libraries
            let startIndex = 0;
            let totalForView = 0;
            let pageNum = 0;
            let viewKept = 0;
            ...
            do {
                ...
                if (!result.success || !result.data) {
                    if (pageNum === 0) {
                        // First page failed — entire view is inaccessible
                ...
                const data = result.data as { Items?: JellyfinMediaItem[]; TotalRecordCount?: number };
        ...
        for (const { item, viewId } of allItems) {
            ...
            await indexJellyfinEmbyItem(integrationId, instance, viewId, item, 'jellyfin');
            indexedItems++;
        ...
        if (syncState.cancelled) {
            ...
            invalidateSystemSettings('media-search-sync');
            logger.warn(`[LibrarySync] Jellyfin sync completed with errors: integrationId=${integrationId}, failed=[${failedViews.map(v => v.name).join(', ')}]`);
        } else {
            updateSyncStatus(integrationId, {
                syncStatus: 'completed',
                lastSyncCompleted: new Date().toISOString(),
