 ...
 * Canonical location: src/settings/navigation/
 * Moved from src/components/sidebar/settingsMenuConfig.ts
 */

import type { DashboardEditContextValue } from '../../context/DashboardEditContext';
...
export const adminSettingsCategories: SidebarSettingsCategory[] = [
    ...
    },
    {
        ...
        children: [
            ...
            { id: 'oidc', label: 'OpenID Connect', icon: 'KeyRound' },
...
/**
 * Navigation guard for dashboard edit mode.
 * 
 * Pure function — no React imports or hooks.
 * Callers handle their own post-guard UI side-effects
 * (e.g., sidebar mode switch, menu close).
 * 
 * @returns 'proceed' if navigation should continue, 'blocked' if the guard intercepted it.
 */
export function guardedNavigate(
    destination: string,
    dashboardEdit: DashboardEditContextValue | null
): 'proceed' | 'blocked' {
    ...
    // Edit mode active but no unsaved changes — exit edit mode first
