...
const EmbyForm: React.FC<EmbyFormProps> = ({
    ...
    const handleConnect = async () => {
        ...
        try {
            const data = await api.post<{ success: boolean; error?: string; accessToken?: string; userId?: string; username?: string }>('/api/integrations/emby/authenticate', {
                url: config.url,
                username,
                password,
            });
                ...
                <div className="flex items-start gap-3 p-3 rounded-lg bg-warning/10">
                    ...
                    <div className="text-sm">
                        ...
                        <p className="text-theme-secondary mt-1">
                            Your Emby session has expired. Please re-enter your credentials to restore connectivity.
                        </p>
                    </div>
                </div>
