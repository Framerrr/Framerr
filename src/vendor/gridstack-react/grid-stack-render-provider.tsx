...
export interface GridStackRenderProviderProps {
    /** Class name for the grid container element */
...
export function GridStackRenderProvider({
    children,
    containerClassName
}: PropsWithChildren<GridStackRenderProviderProps>) {
    const {
    ...
    const renderCBFn = useCallback(
        (element: HTMLElement, widget: GridStackWidget & { grid?: GridStack }) => {
            if (widget.id && widget.grid) {
                ...
                if ((widget as any).lazyLoad || (optionsRef.current as any).lazyLoad) {
    ...
    const initGrid = useCallback(() => {
        if (containerRef.current) {
            (GridStack as any).renderCB = renderCBFn;
