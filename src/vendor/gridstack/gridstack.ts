...
interface GridCSSStyleSheet extends CSSStyleSheet {
  ...
  _colGenerated?: number; // internal tracker of column count for which width rules were generated
}
...
export class GridStack {
  ...
  /** @internal */
  public _gsEventHandler: { [key: string]: GridStackEventHandlerCallback } = {};
  /** @internal */
  ...
  public makeSubGrid(el: GridItemHTMLElement, ops?: GridStackOptions, nodeToAdd?: GridStackNode, saveContent = true): GridStack {
    ...
    if (!subGrid) return this; // failed to create subGrid
  ...
  protected _updateContainerHeight(): GridStack {
    ...
    // FRAMERR: Remove inline min-height FIRST so all getComputedStyle calls below
    // read the true CSS stylesheet value, not a stale inline value from a previous call.
    // Without this, the check below inflates `row` to the old inline value, creating
    // a circular loop where min-height can never shrink.
    this.el.style.removeProperty('min-height');
    this.el.style.removeProperty('height');
    this.el.offsetHeight; // force reflow so getComputedStyle returns fresh CSS values
    ...
    // Set inline min-height based on content rows, respecting CSS min-height
    if (row) {
      const jsMinHeight = row * cellHeight;
      const cssMin = Utils.parseHeight(getComputedStyle(this.el)['minHeight']);
      // Only set inline min-height if JS value exceeds CSS value
      if (cssMin.h > 0 && cssMin.unit === unit && cssMin.h >= jsMinHeight) {
        // CSS min-height is sufficient — no inline override needed
      } else {
        this.el.style.minHeight = jsMinHeight + unit;
      }
    }
  ...
  protected _setupAcceptWidget(): GridStack {
    ...
    let onDrag = (event: DragEvent, el: GridItemHTMLElement, helper: GridItemHTMLElement) => {
      ...
      // FRAMERR: Force layout recalculation before reading rects.
      // iOS Safari returns stale getBoundingClientRect() values after DOM reflows
      // (e.g. grid height growth past viewport), causing position miscalculation
      // and silent collision detection failure. Reading offsetHeight forces a layout flush.
      this.el.offsetHeight; // eslint-disable-line @typescript-eslint/no-unused-expressions
      ...
      if (node._temporaryRemoved) {
        ...
        // For empty grids, force position to top since compaction will place widget there anyway
        // This ensures the placeholder is visible when dragging anywhere on an empty grid
    ...
    dd.droppable(this.el, {
      ...
      .on(this.el, 'dropover', (event: Event, el: GridItemHTMLElement, helper: GridItemHTMLElement) => {
        ...
        if (!node.grid) {
          ...
          // FRAMERR: Set node.grid so the re-entry guard at line 2207 works for external drags.
          // Without this, iOS WebKit's false dropover events (from pointer re-dispatching) bypass
          // the guard and reset _temporaryRemoved=true, ripping the widget out of the engine.
          node.grid = this;
          el.gridstackNode = node;
        }
        ...
        if (node.grid && node.grid !== this) {
          ...
          // FRAMERR: Clamp widget dimensions to grid column count. Without this, external widgets
          // with data-gs-w values designed for 24-column desktop (e.g. w=12) would overflow a 4-column
          // mobile grid, causing directionCollideCoverage to fail due to nonsensical overlap ratios.
          this.engine.nodeBoundFix(node);
        }
      ...
      .on(this.el, 'drop', (event, el: GridItemHTMLElement, helper: GridItemHTMLElement) => {
        ...
        if (helper !== el) {
          // FRAMERR MODIFICATION: FLIP animation - helper "flies" to placeholder position
          // First: record helper position, Last: get placeholder position, Invert & Play: animate
          const crossfadeHelper = (helperEl: HTMLElement) => {
            // FIRST: Get helper's current position
            const helperRect = helperEl.getBoundingClientRect();

            // LAST: Get placeholder's position (where widget will appear)
            const placeholderEl = this.placeholder;
            if (!placeholderEl) {
              // Fallback: just fade out in place if no placeholder
              helperEl.style.transition = 'opacity 200ms ease-out';
              helperEl.style.opacity = '0';
              setTimeout(() => helperEl.remove(), 220);
              return;
            }

            const placeholderRect = placeholderEl.getBoundingClientRect();

            // Fix helper at current position (FIRST position)
            helperEl.style.position = 'fixed';
            helperEl.style.left = `${helperRect.left}px`;
            helperEl.style.top = `${helperRect.top}px`;
            helperEl.style.width = `${helperRect.width}px`;
            helperEl.style.height = `${helperRect.height}px`;
            helperEl.style.pointerEvents = 'none';
            helperEl.style.zIndex = '9999';
            helperEl.style.margin = '0'; // Reset any margins

            // INVERT & PLAY: Animate to placeholder position (LAST position)
            requestAnimationFrame(() => {
              // Add transition for position + size + opacity
              helperEl.style.transition = 'left 200ms ease-out, top 200ms ease-out, width 200ms ease-out, height 200ms ease-out, opacity 150ms ease-out 100ms';

              // Animate to placeholder position (PLAY)
              helperEl.style.left = `${placeholderRect.left}px`;
              helperEl.style.top = `${placeholderRect.top}px`;
              helperEl.style.width = `${placeholderRect.width}px`;
              helperEl.style.height = `${placeholderRect.height}px`;

              // Fade out slightly delayed so widget can start rendering
              requestAnimationFrame(() => {
                helperEl.style.opacity = '0';
              });
  ...
  protected _onStartMoving(el: GridItemHTMLElement, event: Event, ui: DDUIData, node: GridStackNode, cellWidth: number, cellHeight: number): void {
    ...
    if (node._temporaryRemoved) {
      // FRAMERR: Handle both 'dropover' (initial entry) and 'drag' (re-entry after touch jitter leave)
      // On iOS touch, pointer events can briefly leave the grid boundary causing _leave() → _temporaryRemoved=true.
      // When onDrag re-registers, _onStartMoving is called with event.type='drag', not 'dropover'.
    ...
    // FRAMERR: For external drag, cacheRects sets node._rect based on the node's GRID position
    // (from addNode), but the drag helper may be at a different pixel position. directionCollideCoverage
    // uses node._rect as the "starting position" to compute drag direction and >50% overlap.
    // If _rect doesn't match the actual helper position, the coverage computation fails and widgets
    // never get pushed. Override _rect with the actual UI pixel position.
    if (node._isExternal && ui.position) {
      const mTop = this.opts.marginTop as number || 0;
      const mLeft = this.opts.marginLeft as number || 0;
      const mRight = this.opts.marginRight as number || 0;
      const mBottom = this.opts.marginBottom as number || 0;
      node._rect = {
        x: ui.position.left + mLeft,
        y: ui.position.top + mTop,
        w: node.w * cellWidth - mLeft - mRight,
        h: node.h * cellHeight - mTop - mBottom,
      };
    }
  ...
  protected _dragOrResize(el: GridItemHTMLElement, event: MouseEvent, ui: DDUIData, node: GridStackNode, cellWidth: number, cellHeight: number): void {
    ...
    let moveResult = this.engine.moveNodeCheck(node, { ...p, cellWidth, cellHeight, rect, resizing });
    if (moveResult) {
      node._lastUiPosition = ui.position;
    ...
    // FRAMERR: Always update node._rect to current drag position, even when moveNodeCheck fails.
    // directionCollideCoverage uses _rect as "where I started" to compute movement direction.
    // If _rect is stale (from a previous successful move), the direction calculation becomes wrong,
    // causing moveNodeCheck to keep failing in a feedback loop (the WebKit repeated-drag bug).
    node._rect = { ...rect };
  }
  ...
  protected _leave(el: GridItemHTMLElement, helper?: GridItemHTMLElement): void {
    ...
    // FRAMERR: Guard against false leave during external drag on iOS.
    // After grid layout changes (widgets pushed), iOS fires false leave events
    // from various event paths. Use the last known touch position to verify
    // the finger actually left the grid before processing the leave.
    if (node._isExternal && DDManager.lastTouchX != null) {
      const rect = this.el.getBoundingClientRect();
      if (DDManager.lastTouchX >= rect.left && DDManager.lastTouchX <= rect.right &&
        DDManager.lastTouchY >= rect.top && DDManager.lastTouchY <= rect.bottom) {
        return; // Touch still inside grid — suppress false leave
      }
    }
    helper = helper || el;
