...
export class DDDroppable extends DDBaseImplement implements HTMLElementExtendOpt<DDDroppableOpt> {
  ...
  protected _mouseLeave(e: MouseEvent, calledByEnter = false): void {
    ...
    // FRAMERR: Guard against false mouseleave on iOS touch devices.
    // After grid layout changes (e.g. widgets pushed down during drag), iOS fires
    // false mouseleave events even though the finger is still over the grid.
    // Use last known touch position to verify the finger actually left.
    if (isTouch && !calledByEnter && DDManager.lastTouchX != null) {
      const rect = this.el.getBoundingClientRect();
      const x = DDManager.lastTouchX;
      const y = DDManager.lastTouchY;
      if (x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom) {
        return; // Touch is still inside — suppress false leave
      }
    }
